...
(function () {
  ...
  const FONT_FAMILY_PATTERN = /Font Awesome ([5 ]*)(Solid|Regular|Light|Duotone|Brands|Free|Pro|Kit).*/; // TODO: do we need to handle font-weight for kit SVG pseudo-elements?
  ...
  function isPrivateUnicode (iconName) {
    if (iconName.length !== 1) {
      ...
      const cp = codePointAt(iconName, 0);
  ...
  function resolveCustomIconVersion () {
    const kitConfig = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    const iconName = arguments.length > 1 ? arguments[1] : undefined;

    if (iconName && isPrivateUnicode(iconName)) {
      if (kitConfig && kitConfig.iconUploads) {
        const iconUploads = kitConfig.iconUploads;
        const descriptiveIconName = Object.keys(iconUploads).find(function (key) {
  ...
  function findIcon (iconName, prefix) {
    return new picked(function (resolve, reject) {
      ...
      let kitToken = null;
      const iconVersion = resolveCustomIconVersion(WINDOW.FontAwesomeKitConfig, iconName);
